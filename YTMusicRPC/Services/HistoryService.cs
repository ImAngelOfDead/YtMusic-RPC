...
namespace YTMusicRPC.Services
{
    ...
    {
        private readonly static string _historyFolderPath = Path.Combine(Path.GetDirectoryName(ConfigManager.ConfigFilePath) ?? string.Empty, "history");
        ...
        public async Task<List<object>> GetHistory(){
            ...
            await using (var jsonReader = new JsonTextReader(streamReader))
            {
                ...
                {
                    while (jsonReader.Read() && jsonReader.TokenType != JsonToken.EndArray && historyEntries.Count < maxEntries)
                    {
